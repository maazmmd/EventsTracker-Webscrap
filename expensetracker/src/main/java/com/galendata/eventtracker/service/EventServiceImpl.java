...
public class EventServiceImpl implements EventService {
	...
	public void saveAll() {
		String url;
		url = getUrl(webScrapTag1);
		List<Events> events1 = parseWebPage1(url);
		eventRepository.saveAll(events1);

		url = getUrl(webScrapTag2);
		List<Events> events2 = parseWebPage2(url);
		eventRepository.saveAll(events2);
	}

	public String getUrl(String webPageTag) {
		String webScrapPage = null;
		try {
			...
			for (int temp = 0; temp < nList.getLength(); temp++) {
				...
				if (nNode.getNodeType() == Node.ELEMENT_NODE) {
					...
					if (webPageTag.equals("website1"))
						webScrapPage = eElement.getElementsByTagName(webPageTag).item(0).getTextContent();
					else
						webScrapPage = eElement.getElementsByTagName(webPageTag).item(0).getTextContent();
				}
			}
